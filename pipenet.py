...
from modeling.modeling_pipenet import *
...
def train(args):
    ...
    if True:
        ...
        dataset = LM_QAPipeNet_DataLoader(args, args.train_statements, args.train_adj,
        ...
        model = LM_QAPipeNet(args, args.encoder, k=args.k, n_ntype=4, n_etype=args.num_relation, n_concept=concept_num,
        ...
        for epoch_id in range(args.n_epochs):
            ...
            for qids, labels, *input_data in dataset.train():
                ...
                for a in range(0, bs, args.mini_batch_size):
                    ...
                    #print(qids[a], logits, labels[a:b], loss, total_loss)
            ...
            if dev_acc >= best_dev_acc:
                ...
                if args.save_model:
                    torch.save([model.state_dict(), args], f"{model_path}.best")
                    print(f'model saved to {model_path}.best.{epoch_id}')
            model.train()
            start_time = time.time()
            if epoch_id > args.unfreeze_epoch and epoch_id - best_dev_epoch >= args.max_epochs_before_stop:
                break
        print(f'best epoch id: ', best_dev_epoch)
...
def eval_detail(args):
    ...
    model = LM_QAPipeNet(old_args, old_args.encoder, k=old_args.k, n_ntype=4, n_etype=old_args.num_relation, n_concept=concept_num,
