...
class MultiGPUSparseAdjDataBatchGenerator(object):
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            #edge_score_all:  nested list of shape (n_samples, num_choice), where each entry is tensor[E, ]
...
def load_sparse_adj_data_with_contextnode(adj_pk_path, max_node_num, num_choice, args):
    ...
    if use_cache:
        with open(cache_path, 'rb') as f:
            adj_lengths_ori, edge_lengths, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type, half_n_rel = pickle.load(f)
    else:
        ...
        edge_lengths = torch.zeros((n_samples,), dtype=torch.long)
        ...
        for idx, _data in tqdm(enumerate(adj_concept_pairs), total=n_samples, desc='loading adj matrices'):
            ...
            adj, concepts, qm, am, cid2score = _data[0], _data[1], _data[2], _data[3], _data[4]
            ...
            if (cid2score is not None):
                ...
                for _j_ in range(num_concept):
                    ...
                    node_scores[idx, _j_, 0] = torch.tensor(1.0/cid2score[_cid])
        ...
        with open(cache_path, 'wb') as f:
            pickle.dump([adj_lengths_ori, edge_lengths,  concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type, half_n_rel], f)


    ori_adj_mean  = adj_lengths_ori.float().mean().item()
    ori_adj_sigma = np.sqrt(((adj_lengths_ori.float() - ori_adj_mean)**2).mean().item())

    print('| ori_adj_len: mu {:.2f} sigma {:.2f} | adj_len: {:.2f} | edge_len: {:.2f} |'.format(ori_adj_mean, ori_adj_sigma, adj_lengths.float().mean().item(), edge_lengths.float().mean().item()) +
