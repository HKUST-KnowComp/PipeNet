...
def concepts_to_adj_matrices_2hop_all_pair(data, prune_rate=0.9):
    ...
    sample_len = int(len(extra_nodes_new) * (1-prune_rate))
    extra_nodes_new_choice = random.sample(extra_nodes_new, sample_len)

    schema_graph = sorted(q_only_ids) + sorted(ac_ids) + extra_nodes_new_choice
